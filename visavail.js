				...
				selection.each(function drawGraph(dataset) {
					...
					dataset.forEach(function (d) {
						d.data.forEach(function (d1) {
							// TODO day each start time inner each measure
							...
							// TODO day each finish time inner each measure
					...
					dataset.forEach(function (series, seriesI) {
						...
						dataset[seriesI].disp_data = tmpData;   // TODO ?
					...
					function redrawTooltipWhenOver(obj, dataset, pageX, pageY, d, i) {
						console.log('---> redrawTooltipWhenOver()','obj', obj, 'pageX', pageX, 'pageY', pageY, 'd', d, 'i', i, 'dataset', dataset )
						if (options.tooltip.hover_zoom.enabled) {
							d3.select(obj).transition()
								.duration(options.tooltip.duration)
								.attr('x', function (d) {
						...
						div.html(function () {
							...
							if (options.tooltip.description) {	// TODO
								var series = dataset.filter(
									function (series) {
										return series.disp_data.indexOf(d) >= 0;
									}
								...
								// TODO
								// if (series  && series.dataset[i][3]) {
									// output += ' ' + series.dataset[i][3] + ' ';
								// }
